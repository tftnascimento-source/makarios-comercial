"use client";

import { useState, useMemo, useCallback } from "react";
import { Package, Search, FileDown, FileText, ChevronDown, ChevronUp } from "lucide-react";
import { formatBRL } from "@/lib/utils";

type ProdutoRow = {
  cProd: string;
  xProd: string;
  totalQtd: number;
  totalValor: number;
  totalNotas: number;
  totalClientes: number;
};

interface Props {
  produtos: ProdutoRow[];
  periodos: string[];
  produtosPorPeriodo: (ProdutoRow & { periodo: string })[];
  empresas: { id: string; nome: string }[];
  produtosPorEmpresa: (ProdutoRow & { empresaId: string })[];
}

function periodoLabel(p: string) {
  const [ano, mes] = p.split("-");
  const nomes = ["Janeiro","Fevereiro","Março","Abril","Maio","Junho",
    "Julho","Agosto","Setembro","Outubro","Novembro","Dezembro"];
  return `${nomes[Number(mes) - 1] ?? mes} / ${ano}`;
}

const ABC_STYLE = {
  A: { badge: "bg-green-100 text-green-800 border-green-200", bar: "bg-green-500" },
  B: { badge: "bg-amber-100 text-amber-800 border-amber-200", bar: "bg-amber-400" },
  C: { badge: "bg-gray-100 text-gray-600 border-gray-200",    bar: "bg-gray-400" },
};

export default function ProdutosClient({
  produtos,
  periodos,
  produtosPorPeriodo,
  empresas,
  produtosPorEmpresa,
}: Props) {
  const [periodoBusca, setPeriodoBusca] = useState("todos");
  const [empresaFiltro, setEmpresaFiltro] = useState("todos");
  const [busca, setBusca] = useState("");
  const [showAll, setShowAll] = useState(false);
  const [exportando, setExportando] = useState<"xlsx" | "pdf" | null>(null);

  // Build filtered product list
  const filtrado = useMemo<ProdutoRow[]>(() => {
    let base: ProdutoRow[];

    if (periodoBusca !== "todos" && empresaFiltro !== "todos") {
      // Both filters: intersect per-period and per-empresa (need server data combining both)
      // Approximate: filter produtosPorPeriodo by empresa via produtosPorEmpresa lookup
      // Since we don't have a 3-way table, fall back to period-only for now
      const map = new Map<string, ProdutoRow>();
      const empresaProds = new Set(
        produtosPorEmpresa
          .filter((r) => r.empresaId === empresaFiltro)
          .map((r) => r.cProd)
      );
      for (const row of produtosPorPeriodo) {
        if (row.periodo !== periodoBusca) continue;
        if (!empresaProds.has(row.cProd)) continue;
        const existing = map.get(row.cProd);
        if (existing) {
          existing.totalValor += row.totalValor;
          existing.totalQtd   += row.totalQtd;
          existing.totalNotas += row.totalNotas;
        } else {
          map.set(row.cProd, { ...row });
        }
      }
      base = [...map.values()];
    } else if (periodoBusca !== "todos") {
      const map = new Map<string, ProdutoRow>();
      for (const row of produtosPorPeriodo) {
        if (row.periodo !== periodoBusca) continue;
        const existing = map.get(row.cProd);
        if (existing) {
          existing.totalValor += row.totalValor;
          existing.totalQtd   += row.totalQtd;
          existing.totalNotas += row.totalNotas;
        } else {
          map.set(row.cProd, { ...row });
        }
      }
      base = [...map.values()];
    } else if (empresaFiltro !== "todos") {
      base = produtosPorEmpresa.filter((r) => r.empresaId === empresaFiltro);
    } else {
      base = produtos;
    }

    const sorted = [...base].sort((a, b) => b.totalValor - a.totalValor);

    if (!busca.trim()) return sorted;
    const q = busca.toLowerCase();
    return sorted.filter(
      (r) => r.xProd.toLowerCase().includes(q) || (r.cProd ?? "").toLowerCase().includes(q)
    );
  }, [produtos, periodoBusca, empresaFiltro, busca, produtosPorPeriodo, produtosPorEmpresa]);

  // Enrich with ABC
  const enriched = useMemo(() => {
    const totalValor = filtrado.reduce((s, r) => s + r.totalValor, 0);
    let cumulative = 0;
    return filtrado.map((item) => {
      const pct = totalValor > 0 ? (item.totalValor / totalValor) * 100 : 0;
      cumulative += pct;
      const cls: "A" | "B" | "C" = cumulative <= 80 ? "A" : cumulative <= 95 ? "B" : "C";
      return { ...item, pct, cumulative, cls, totalValor_: totalValor };
    });
  }, [filtrado]);

  const displayed = showAll ? enriched : enriched.slice(0, 20);
  const totalValor = enriched.reduce((s, r) => s + r.totalValor, 0);
  const counts = { A: 0, B: 0, C: 0 };
  for (const r of enriched) counts[r.cls]++;

  const handleExport = useCallback(async (tipo: "xlsx" | "pdf") => {
    setExportando(tipo);
    try {
      const params = new URLSearchParams();
      if (periodoBusca !== "todos") params.set("periodo", periodoBusca);
      if (empresaFiltro !== "todos") params.set("empresaId", empresaFiltro);
      const url = tipo === "pdf"
        ? `/api/exportar/produtos-pdf?${params}`
        : `/api/exportar/produtos?${params}`;
      const res = await fetch(url);
      if (!res.ok) throw new Error("Erro ao exportar");
      const blob = await res.blob();
      const a = document.createElement("a");
      a.href = URL.createObjectURL(blob);
      const cd = res.headers.get("Content-Disposition") ?? "";
      const match = cd.match(/filename="([^"]+)"/);
      a.download = match?.[1] ?? `makarios-produtos.${tipo === "pdf" ? "pdf" : "xlsx"}`;
      a.click();
      URL.revokeObjectURL(a.href);
    } catch {
      // silent
    } finally {
      setExportando(null);
    }
  }, [periodoBusca, empresaFiltro]);

  return (
    <div className="space-y-5">
      {/* Header */}
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          <h1 className="text-xl font-semibold text-[var(--color-mk-black)]">Produtos — Curva ABC</h1>
          <p className="text-sm text-[var(--color-mk-gray)] mt-0.5">
            Análise de receita por produto · Todos os clientes consolidados
          </p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          {/* Search */}
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-[var(--color-mk-gray)]" />
            <input
              value={busca}
              onChange={(e) => setBusca(e.target.value)}
              placeholder="Buscar produto…"
              className="pl-9 pr-3 py-1.5 rounded-lg border border-[var(--color-border)] bg-white text-sm focus:outline-none focus:ring-2 focus:ring-[var(--color-mk-gold)]/30 w-44"
            />
          </div>
          {/* Empresa filter */}
          {empresas.length > 1 && (
            <select
              value={empresaFiltro}
              onChange={(e) => setEmpresaFiltro(e.target.value)}
              className="rounded-lg border border-[var(--color-border)] bg-white px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--color-mk-gold)]/30"
            >
              <option value="todos">Todas as empresas</option>
              {empresas.map((e) => (
                <option key={e.id} value={e.id}>{e.nome}</option>
              ))}
            </select>
          )}
          {/* Period filter */}
          <select
            value={periodoBusca}
            onChange={(e) => { setPeriodoBusca(e.target.value); setShowAll(false); }}
            className="rounded-lg border border-[var(--color-border)] bg-white px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--color-mk-gold)]/30"
          >
            <option value="todos">Todos os períodos</option>
            {periodos.map((p) => (
              <option key={p} value={p}>{periodoLabel(p)}</option>
            ))}
          </select>
          {/* Export */}
          <button
            onClick={() => handleExport("xlsx")}
            disabled={exportando !== null || enriched.length === 0}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-[var(--color-border)] bg-white text-sm text-[var(--color-mk-gray-dark)] hover:bg-[var(--color-muted)] transition-colors disabled:opacity-50"
            title="Exportar Excel"
          >
            <FileDown className="h-3.5 w-3.5" />
            {exportando === "xlsx" ? "…" : "Excel"}
          </button>
          <button
            onClick={() => handleExport("pdf")}
            disabled={exportando !== null || enriched.length === 0}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-[var(--color-border)] bg-white text-sm text-[var(--color-mk-gray-dark)] hover:bg-[var(--color-muted)] transition-colors disabled:opacity-50"
            title="Exportar PDF"
          >
            <FileText className="h-3.5 w-3.5" />
            {exportando === "pdf" ? "…" : "PDF"}
          </button>
        </div>
      </div>

      {/* KPI strip */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: "Produtos", value: String(enriched.length), sub: "no mix" },
          { label: "Receita Total", value: formatBRL(totalValor), sub: "acumulada" },
          { label: "Classe A", value: String(counts.A), sub: `${enriched.length > 0 ? ((counts.A / enriched.length) * 100).toFixed(0) : 0}% do mix · ~80% receita` },
          { label: "Notas / Pedidos", value: String(enriched.reduce((s, r) => s + r.totalNotas, 0)), sub: "ocorrências" },
        ].map((k) => (
          <div key={k.label} className="bg-white rounded-xl border border-[var(--color-border)] px-5 py-3.5">
            <p className="text-xs font-semibold text-[var(--color-mk-gray)] uppercase tracking-wide">{k.label}</p>
            <p className="text-xl font-bold text-[var(--color-mk-black)] mt-1">{k.value}</p>
            <p className="text-xs text-[var(--color-mk-gray)] mt-0.5">{k.sub}</p>
          </div>
        ))}
      </div>

      {/* Table */}
      <div className="bg-white rounded-xl border border-[var(--color-border)] overflow-hidden">
        {enriched.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 gap-3 text-[var(--color-mk-gray)]">
            <Package className="h-10 w-10" />
            <p className="text-sm font-medium">Nenhum produto encontrado</p>
            <p className="text-xs">Importe NF-e para registrar itens automaticamente.</p>
          </div>
        ) : (
          <>
            {/* ABC class summary */}
            <div className="flex items-center gap-3 px-5 pt-4 pb-3 flex-wrap border-b border-[var(--color-border)]">
              {(["A", "B", "C"] as const).map((cls) => (
                <span key={cls} className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold border ${ABC_STYLE[cls].badge}`}>
                  Classe {cls} · {counts[cls]} produto{counts[cls] !== 1 ? "s" : ""}
                </span>
              ))}
            </div>

            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[var(--color-border)] bg-[var(--color-muted)]">
                    <th className="text-center px-4 py-2.5 font-semibold text-[var(--color-mk-gray)] text-xs uppercase tracking-wide w-14">ABC</th>
                    <th className="text-left px-5 py-2.5 font-semibold text-[var(--color-mk-gray)] text-xs uppercase tracking-wide">Produto</th>
                    <th className="text-right px-5 py-2.5 font-semibold text-[var(--color-mk-gray)] text-xs uppercase tracking-wide hidden sm:table-cell">Qtd. Vendida</th>
                    <th className="text-right px-5 py-2.5 font-semibold text-[var(--color-mk-gray)] text-xs uppercase tracking-wide">Receita</th>
                    <th className="text-right px-5 py-2.5 font-semibold text-[var(--color-mk-gray)] text-xs uppercase tracking-wide hidden md:table-cell">% Total</th>
                    <th className="px-5 py-2.5 font-semibold text-[var(--color-mk-gray)] text-xs uppercase tracking-wide hidden lg:table-cell">Participação</th>
                    <th className="text-right px-5 py-2.5 font-semibold text-[var(--color-mk-gray)] text-xs uppercase tracking-wide hidden md:table-cell">% Acum.</th>
                    <th className="text-center px-5 py-2.5 font-semibold text-[var(--color-mk-gray)] text-xs uppercase tracking-wide hidden lg:table-cell">Clientes</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[var(--color-border)]">
                  {displayed.map((item, idx) => (
                    <tr
                      key={`${item.cProd}-${idx}`}
                      className={`hover:bg-[var(--color-muted)] transition-colors ${item.cls === "A" ? "bg-green-50/30" : ""}`}
                    >
                      <td className="px-4 py-3 text-center">
                        <span className={`inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold border ${ABC_STYLE[item.cls].badge}`}>
                          {item.cls}
                        </span>
                      </td>
                      <td className="px-5 py-3">
                        <p className="font-medium text-[var(--color-mk-black)]">{item.xProd}</p>
                        <p className="text-xs text-[var(--color-mk-gray)] font-mono">{item.cProd}</p>
                      </td>
                      <td className="px-5 py-3 text-right text-[var(--color-mk-gray)] hidden sm:table-cell">
                        {Number(item.totalQtd).toLocaleString("pt-BR", { maximumFractionDigits: 2 })}
                      </td>
                      <td className="px-5 py-3 text-right font-semibold text-[var(--color-mk-black)]">
                        {formatBRL(item.totalValor)}
                      </td>
                      <td className="px-5 py-3 text-right text-[var(--color-mk-gray)] hidden md:table-cell">
                        {item.pct.toFixed(1)}%
                      </td>
                      <td className="px-5 py-3 hidden lg:table-cell">
                        <div className="w-full bg-gray-100 rounded-full h-2">
                          <div
                            className={`h-2 rounded-full ${ABC_STYLE[item.cls].bar}`}
                            style={{ width: `${Math.min(item.pct * 4, 100)}%` }}
                          />
                        </div>
                      </td>
                      <td className="px-5 py-3 text-right hidden md:table-cell">
                        <span className={`text-xs font-semibold ${
                          item.cumulative <= 80 ? "text-green-700" :
                          item.cumulative <= 95 ? "text-amber-600" : "text-gray-500"
                        }`}>
                          {item.cumulative.toFixed(1)}%
                        </span>
                      </td>
                      <td className="px-5 py-3 text-center text-[var(--color-mk-gray)] text-xs hidden lg:table-cell">
                        {item.totalClientes}
                      </td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>

            {enriched.length > 20 && (
              <div className="border-t border-[var(--color-border)] px-5 py-3 flex justify-center">
                <button
                  onClick={() => setShowAll(!showAll)}
                  className="inline-flex items-center gap-1.5 text-sm text-[var(--color-mk-gold)] hover:text-[var(--color-mk-gold-dark)] font-medium transition-colors"
                >
                  {showAll ? (
                    <><ChevronUp className="h-4 w-4" /> Mostrar menos</>
                  ) : (
                    <><ChevronDown className="h-4 w-4" /> Ver todos os {enriched.length} produtos</>
                  )}
                </button>
              </div>
            )}
          </>
        )}
      </div>

      {/* ABC legend */}
      {enriched.length > 0 && (
        <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-muted)] px-5 py-4">
          <p className="text-xs font-semibold text-[var(--color-mk-black)] mb-2">Como ler a Curva ABC</p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs text-[var(--color-mk-gray-dark)]">
            <div className="flex items-start gap-2">
              <span className="inline-flex w-5 h-5 rounded-full bg-green-100 text-green-800 border border-green-200 items-center justify-center font-bold shrink-0 mt-0.5">A</span>
              <span><strong>Itens estratégicos</strong> — representam ~80% da receita total. Priorizar estoque, mix e negociação de preço.</span>
            </div>
            <div className="flex items-start gap-2">
              <span className="inline-flex w-5 h-5 rounded-full bg-amber-100 text-amber-800 border border-amber-200 items-center justify-center font-bold shrink-0 mt-0.5">B</span>
              <span><strong>Itens intermediários</strong> — contribuem com ~15% adicionais. Monitorar sazonalidade e oportunidades de upsell.</span>
            </div>
            <div className="flex items-start gap-2">
              <span className="inline-flex w-5 h-5 rounded-full bg-gray-100 text-gray-600 border border-gray-200 items-center justify-center font-bold shrink-0 mt-0.5">C</span>
              <span><strong>Cauda longa</strong> — muitos itens, baixo volume individual. Avaliar custo de manutenção no mix.</span>
            </div>
          </div>
        </div>
      )}
    </div>
  );
}
